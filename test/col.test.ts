...
describe("collision", () => {
    ...
    describe("line", () => {
        ...
        describe("line", () => {
            ...
            it("0 length lines act like points: false", () => {
                const l1 = col.constructWallEntry({sx: -2, sy: -1, ex: -2, ey: -1})
                const l2 = col.constructWallEntry({sx: -2, sy: -5, ex: -2, ey: -5})
                expect(col.collide(l1,l2)).toBeFalsy()
            })
            it("0 length lines act line points: true", () => {
                const l1 = col.constructWallEntry({sx: -2, sy: -1, ex: -2, ey: -1})
                const l2 = col.constructWallEntry({sx: -2, sy: -1, ex: -2, ey: -1})
                expect(col.collide(l1,l2)).toBeTruthy()
            })
            it("parallel", () => {
                const l1 = col.constructWallEntry({sx: 0, sy: 0, ex: 0, ey: 10})
                const l2 = col.constructWallEntry({sx: 5, sy: 0, ex: 5, ey: 10})
                expect(col.collide(l1,l2)).toBeFalsy()
            })
